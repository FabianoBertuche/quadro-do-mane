...
export default function DailyRoutineAdminPage() {
  ...
  const { data: logs, isLoading, isError } = useQuery<RoutineLog[]>({
        ...
        <div className="md:col-span-2 p-6 rounded-2xl bg-card border border-border shadow-sm flex flex-col justify-center">
          ...
          <p className="text-sm text-muted-foreground">
            {isLoading ? 'Carregando logs...' : 
             logs && logs.length > 0 ? `Encontrados ${logs.length} registros de conclusão para o período.` : 
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-muted/50 border-b border-border">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-muted-foreground">Data de Conclusão</th>
              ...
              {isLoading ? (
                ...
                logs.map((log, idx) => (
                  <tr key={idx} className="hover:bg-muted/30 transition-colors group">
                    <td className="px-6 py-4 font-medium text-foreground">{log.employeeName}</td>
                    <td className="px-6 py-4 text-muted-foreground">{log.routineItem}</td>
                    <td className="px-6 py-4 tabular-nums text-muted-foreground">{log.completionDate}</td>
